...
_weight            = "genWeight*weight*getPUWeight(Pileup_nTrueInt)*getLeptonTauFake(4,genPartFlav_1,eta_1)*getLeptonTauFake(4,genPartFlav_2,eta_2)*(genPartFlav_1==5 ? 0.89 : 1)*(genPartFlav_2==5 ? 0.89 : 1)"
baseline           = "q_1*q_2<0 && idDecayMode_1==1 && idDecayMode_2==1 && idMVAoldDM2017v2_1>=16 && idMVAoldDM2017v2_2>=16 && idAntiMu_1>=1 && idAntiEle_1>=1 && idAntiMu_2>=1 && idAntiEle_2>=1 && extramuon_veto==0 && extraelec_veto==0"
if "B-E" in era:
...
#     var("m_vis",                                40,     0, 1000, filename="$VAR_zoom" ),
...
samplesD = {
    ...
    'tautau': ( 'Tau',             "Tau_Run2017",     "observed",      ),
}

# SAMPLESET
makeNanoAODSamples('tautau',samplesD,samplesB,samplesS,weight=_weight,binN_weighted=16,blind=blind_dict)
samples = SampleSet(samplesD,samplesB,samplesS,channel='tautau',nanoAOD=True)
#samples.printTable()
...
elif splitTT:      samples.split('TT', [('TTT',"ttbar with real tau_h","genPartFlav_1==5 && genPartFlav_2==5"), ('TTJ',"ttbar other",     "(genPartFlav_1!=5 || genPartFlav_2!=5)")]) #'ttbar j -> tau_h': "genPartFlav_2<5"
#if splitST:     samples.split('ST', [('STT',"single top with real tau_h',"genPartFlav_2==5"), ('STJ',"single top other","genPartFlav_2!=5")])
if splitDY:
  if doFakeFactor: samples.split('DY', [('ZTT',"Z -> tau_{h}tau_{h}",  "genPartFlav_1==5 && genPartFlav_2==5"), ('ZL', "Drell-Yan with l -> tau_{h}","(genPartFlav_1!=5 || genPartFlav_2!=5)")])
  #if doFakeFactor: samples.split('DY', [('ZTT',"Z -> tau_{h}tau_{h}",  "genPartFlav_2==5"), ('ZL',"Drell-Yan with l -> tau_{h}","genPartFlav_2!=5")])
  else:            samples.split('DY', [('ZTT',"Z -> tau_{h}tau_{h}",  "genPartFlav_1==5 && genPartFlav_2==5"), ('ZJ', "Drell-Yan other", "(genPartFlav_1!=5 || genPartFlav_2!=5)")])
samples.printTable()
