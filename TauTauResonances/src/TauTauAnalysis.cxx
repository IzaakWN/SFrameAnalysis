...
TauTauAnalysis::TauTauAnalysis() : SCycleBase(),
    ...
    m_PileupReweightingTool_BtoE( this ),
...
void TauTauAnalysis::BeginInputData( const SInputData& id ) throw( SError ) {
  ...
  m_doMES   = m_MESshift != 0.0 and !m_isData;
  ...
  m_logger << INFO << "dataPUFileName_80p0: " << m_dataPUFileName_80p0 << SLogger::endmsg;
  m_logger << INFO << "dataPUFileName_BtoE: " << m_dataPUFileName_BtoE << SLogger::endmsg;
  m_logger << INFO << "dataPUFileName_F:    " << m_dataPUFileName_F    << SLogger::endmsg;
  ...
  for(int chi = 0; chi < (int)channels_.size(); chi++){
    ...
    DeclareVariable( b_puweightBtoE[ch],        "puweightBtoE",         treeName);
    DeclareVariable( b_puweightF[ch],           "puweightF",            treeName);
    ...
    DeclareVariable( b_gen_ptvis_2[ch],         "gen_ptvis_2",          treeName);
    DeclareVariable( b_gen_mvis_2[ch],          "gen_mvis_2",           treeName);
    DeclareVariable( b_gen_decayMode_2[ch],     "gen_decayMode_2",      treeName);
    ...
    DeclareVariable( b_byIsolationMVArun2v2DBoldDMwLTraw_2[ch],            "byIsolationMVArun2v2DBoldDMwLTraw_2",            treeName);
    DeclareVariable( b_byIsolationMVArun2v1DBnewDMwLTraw_2[ch],            "byIsolationMVArun2v1DBnewDMwLTraw_2",            treeName);
    
    DeclareVariable( b_byVLooseIsolationMVArun2v2DBoldDMwLT_2[ch],         "byVLooseIsolationMVArun2v2DBoldDMwLT_2",         treeName);
    DeclareVariable( b_byLooseIsolationMVArun2v2DBoldDMwLT_2[ch],          "byLooseIsolationMVArun2v2DBoldDMwLT_2",          treeName);
    DeclareVariable( b_byMediumIsolationMVArun2v2DBoldDMwLT_2[ch],         "byMediumIsolationMVArun2v2DBoldDMwLT_2",         treeName);
    DeclareVariable( b_byTightIsolationMVArun2v2DBoldDMwLT_2[ch],          "byTightIsolationMVArun2v2DBoldDMwLT_2",          treeName);
    DeclareVariable( b_byVTightIsolationMVArun2v2DBoldDMwLT_2[ch],         "byVTightIsolationMVArun2v2DBoldDMwLT_2",         treeName);
    DeclareVariable( b_byVVTightIsolationMVArun2v2DBoldDMwLT_2[ch],        "byVVTightIsolationMVArun2v2DBoldDMwLT_2",        treeName);
    
    DeclareVariable( b_byVLooseIsolationMVArun2v1DBnewDMwLT_2[ch],         "byVLooseIsolationMVArun2v1DBnewDMwLT_2",         treeName);
    DeclareVariable( b_byLooseIsolationMVArun2v1DBnewDMwLT_2[ch],          "byLooseIsolationMVArun2v1DBnewDMwLT_2",          treeName);
    DeclareVariable( b_byMediumIsolationMVArun2v1DBnewDMwLT_2[ch],         "byMediumIsolationMVArun2v1DBnewDMwLT_2",         treeName);
    DeclareVariable( b_byTightIsolationMVArun2v1DBnewDMwLT_2[ch],          "byTightIsolationMVArun2v1DBnewDMwLT_2",          treeName);
    DeclareVariable( b_byVTightIsolationMVArun2v1DBnewDMwLT_2[ch],         "byVTightIsolationMVArun2v1DBnewDMwLT_2",         treeName);
    DeclareVariable( b_byVVTightIsolationMVArun2v1DBnewDMwLT_2[ch],        "byVVTightIsolationMVArun2v1DBnewDMwLT_2",        treeName);
    
    DeclareVariable( b_byVVLooseCombinedIsolationDeltaBetaCorr3Hits_2[ch], "byVVLooseCombinedIsolationDeltaBetaCorr3Hits_2", treeName);
    ...
    if(!m_doTight){
      ...
      DeclareVariable( b_flightLengthSig_2[ch],                  "flightLenthSig_2",                 treeName);
    }
    ...
    DeclareVariable( b_pt_ll[ch],               "pt_tt",                treeName);
  ...
  //Book( TH1F("NUP",       "number of partons",  10,   0,  10 ), "checks");
  ...
  if (!m_isData){
    ...
    m_PileupReweightingTool_BtoE.BeginInputData( id, m_dataPUFileName_BtoE, "_BtoE" );
    m_PileupReweightingTool_F.BeginInputData(    id, m_dataPUFileName_F,    "_F"    );
  }else{
    TObject* grl;
  ...
  //m_PileupReweightingTool.drawPUWeight();
...
void TauTauAnalysis::ExecuteEvent( const SInputData&, Double_t ) throw( SError ) {
  ...
  if (m_isData){
    ...
    Hist("npu_float", "checks")->Fill(m_eventInfo.PV_N);
    if(m_eventInfo.PV_N<=0) throw SError( SError::SkipEvent );
  }else{
    //Hist("NUP", "checks")->Fill(m_eventInfo.lheNj);
    for (auto ch: channels_)
  ...
  for( int i = 0; i < m_muon.N; ++i ){
    ...
    if(mymuon.SemileptonicPFIso()/mymuon.pt()>0.50) continue;
  ...
  for ( int i = 0; i <   (m_tau.N); ++i ) {
    ...
    if(!m_isData){
        ...
        else if(m_doLTF and genmatch_2<5){
            taupt *= (1+m_LTFshift);
        }
        else if(m_doJTF and genmatch_2==6){
            taupt *= (1.+m_JTFshift);
        }
    }
  ...
  if(mutau_pair.size()!=0){
    ...
    b_isolated_      = mutau_pair[0].lep_iso<0.50;
    if(m_doTight) b_isolated_ &= (goodTaus[mutau_pair[0].itau].byVLooseIsolationMVArun2v1DBoldDMwLT()==1 or
                                  goodTaus[mutau_pair[0].itau].byVLooseIsolationMVArun2v2DBoldDMwLT()==1 or
                                  goodTaus[mutau_pair[0].itau].byVLooseIsolationMVArun2v1DBnewDMwLT()==1 or
                                  goodTaus[mutau_pair[0].itau].byLooseIsolationMVArun2v1DBoldDMwLT()==1 or
                                  goodTaus[mutau_pair[0].itau].byLooseIsolationMVArun2v2DBoldDMwLT()==1 or
                                  goodTaus[mutau_pair[0].itau].byLooseIsolationMVArun2v1DBnewDMwLT()==1);
    ...
    if(!m_isData and !m_doTight and (goodTaus[mutau_pair[0].itau].byTightIsolationMVArun2v2DBoldDMwLT()==1 or
                                    goodTaus[mutau_pair[0].itau].byTightIsolationMVArun2v1DBnewDMwLT()==1)){
...
void TauTauAnalysis::FillBranches(const std::string& channel, const UZH::Tau& tau, const int gen_match_2,
  ...
  if(m_doRecoilCorr || m_doZpt){
    setGenBosonTLVs(); // only for HTT and DY
  ...
  b_puweightBtoE[ch] = m_isData ? 1.0 : m_PileupReweightingTool_BtoE.getPileUpWeight( b_npu_ );
  b_puweightF[ch]    = m_isData ? 1.0 : m_PileupReweightingTool_F.getPileUpWeight(    b_npu_ );
  ...
  b_byIsolationMVArun2v2DBoldDMwLTraw_2[ch]            = tau.byIsolationMVArun2v2DBoldDMwLTraw();
  b_byVLooseIsolationMVArun2v2DBoldDMwLT_2[ch]         = tau.byVLooseIsolationMVArun2v2DBoldDMwLT();
  b_byLooseIsolationMVArun2v2DBoldDMwLT_2[ch]          = tau.byLooseIsolationMVArun2v2DBoldDMwLT();
  b_byMediumIsolationMVArun2v2DBoldDMwLT_2[ch]         = tau.byMediumIsolationMVArun2v2DBoldDMwLT();
  b_byTightIsolationMVArun2v2DBoldDMwLT_2[ch]          = tau.byTightIsolationMVArun2v2DBoldDMwLT();  
  b_byVTightIsolationMVArun2v2DBoldDMwLT_2[ch]         = tau.byVTightIsolationMVArun2v2DBoldDMwLT();
  b_byVVTightIsolationMVArun2v2DBoldDMwLT_2[ch]        = tau.byVVTightIsolationMVArun2v2DBoldDMwLT();
  
  b_byIsolationMVArun2v1DBnewDMwLTraw_2[ch]            = tau.byIsolationMVArun2v1DBnewDMwLTraw();
  b_byVLooseIsolationMVArun2v1DBnewDMwLT_2[ch]         = tau.byVLooseIsolationMVArun2v1DBnewDMwLT();
  b_byLooseIsolationMVArun2v1DBnewDMwLT_2[ch]          = tau.byLooseIsolationMVArun2v1DBnewDMwLT();
  b_byMediumIsolationMVArun2v1DBnewDMwLT_2[ch]         = tau.byMediumIsolationMVArun2v1DBnewDMwLT();
  b_byTightIsolationMVArun2v1DBnewDMwLT_2[ch]          = tau.byTightIsolationMVArun2v1DBnewDMwLT();  
  b_byVTightIsolationMVArun2v1DBnewDMwLT_2[ch]         = tau.byVTightIsolationMVArun2v1DBnewDMwLT();
  b_byVVTightIsolationMVArun2v1DBnewDMwLT_2[ch]        = tau.byVVTightIsolationMVArun2v1DBnewDMwLT();
  
  b_byVVLooseCombinedIsolationDeltaBetaCorr3Hits_2[ch] = tau.byCombinedIsolationDeltaBetaCorrRaw3Hits()<4.5;
  b_byVLooseCombinedIsolationDeltaBetaCorr3Hits_2[ch]  = tau.byCombinedIsolationDeltaBetaCorrRaw3Hits()<3.5;
  ...
  b_gen_decayMode_2[ch]                                = -1;
  b_gen_ptvis_2[ch]                                    = -1;
  b_gen_mvis_2[ch]                                     = -1;
  if(!m_doTight){
    if(!m_isData and gen_match_2==5) b_gen_decayMode_2[ch] = genDecayMode(tau,b_gen_ptvis_2[ch],b_gen_mvis_2[ch]);
    ...
    b_chargedPionPt_2[ch]                              = -1;
    b_neutralPionPt_2[ch]                              = -1;
    b_chargedIsoPtSum_2[ch]                            = -1;
    b_neutralIsoPtSum_2[ch]                            = -1;
    b_chargedIsoPtSumdR03_2[ch]                        = -1;
    b_neutralIsoPtSumdR03_2[ch]                        = -1;
    b_puCorrPtSum_2[ch]                                = -1;
    b_photonPtSumOutsideSignalCone_2[ch]               = -1;
    b_photonPtSumOutsideSignalConedR03_2[ch]           = -1;
    b_byPhotonPtSumOutsideSignalCone_2[ch]             = -1;
    
    b_nPhoton_2[ch]                                    = -1;
    b_ptWeightedDetaStrip_2[ch]                        = -1;
    b_ptWeightedDphiStrip_2[ch]                        = -1;
    b_ptWeightedDrSignal_2[ch]                         = -1;
    b_ptWeightedDrIsolation_2[ch]                      = -1;
    b_leadingTrackChi2_2[ch]                           = -1;
    b_leadingTrackPt_2[ch]                             = -1;
    b_eRatio_2[ch]                                     = -1;
    b_dxy_Sig_2[ch]                                    = -1;
    b_ip3d_2[ch]                                       = -1;
    b_ip3d_Sig_2[ch]                                   = -1;
    b_hasSecondaryVertex_2[ch]                         = -1;
    b_decayDistMag_2[ch]                               = -1;
    b_flightLengthSig_2[ch]                            = -1;
  }
  ...
  if(m_doRecoilCorr){
    ...
    met_tlv_corrected = m_RecoilCorrector.CorrectPFMETByMeanResolution(  met.px(),           met.py(),
  ...
  if(!m_isData){
    ...
    else if(m_doJTF and gen_match_2==6){ // Jet to tau fake (LTF)
  ...
  TLorentzVector ll_tlv = lep_tlv + tau_tlv;
  b_m_vis[ch]       = ll_tlv.M();
  b_pt_ll[ch]       = (ll_tlv + met.tlv()).Pt();
  b_dR_ll[ch]       = tau_tlv.DeltaR(lep_tlv);
  b_dphi_ll[ch]     = fabs(tau_tlv.DeltaPhi(lep_tlv));
  b_deta_ll[ch]     = fabs(tau_tlv.Eta()-lep_tlv.Eta());
  ...
  if(m_doTight) doSVFit = doSVFit && b_nbtag[ch]>0;
  ...
  if ( doSVFit ){
    ...
    m_SVFitTool.getSVFitMassAndPT(m_sv,pt_tt_sv,met.px(),met.py(),met.cov00(),met.cov10(),met.cov11());
...
void TauTauAnalysis::setGenBosonTLVs(){
  ...
  for( int p=0; p<(int)m_genParticle.pdgId->size(); ++p ){
    ...
    if(((PID==11 or PID==13) and particle.status()==1 and particle.fromHardProcessFinalState()) or
...
int TauTauAnalysis::genMatch(Float_t lep_eta, Float_t lep_phi) {
  ...
  // MATCH hadronically decayed tau
...
int TauTauAnalysis::genDecayMode(const UZH::Tau tau, Float_t& genvispt, Float_t& genvism){
  ...
  for( int p = 0; p < (int)m_genParticle.pdgId->size(); ++p ){
    ...
    Int_t abspdgId = abs(particle.pdgId());
    Int_t isPrompt = particle.isPrompt();
    
    Int_t mother = -999;
    if(particle.nMoth() !=0){
      mother = abs(particle.mother()[0]);
    }
    
    // save gen tau neutrino
    if(particle.status()==1 && abspdgId==16 && mother==15){
    ...
    // skip if not tau
    if(!(particle.status()==2 && abspdgId==15 && isPrompt > 0.5)) continue;
    ...
    for(int idau=0; idau < (int)particle.nDau(); idau++){
      Int_t daughter_PID = abs(particle.dau()[idau]);
      if(daughter_PID==11 || daughter_PID==13 || daughter_PID==15) isleptonic = true;
      if(daughter_PID==111 or daughter_PID==311){
        nNeutral++;
      }
      else if(daughter_PID==211 or daughter_PID==321){
        nCharged++;
      }
      else if(daughter_PID==223){ // 90% omega -> pi+pi-pi0
        nCharged += 2;
        nNeutral++;
      }
    }
    ...
    if(isleptonic==false){
      ...
      if(nCharged>0){
        decaymode[pdgId] = 5*(nCharged-1)+std::min(nNeutral,9);
      //if(nCharged==1){
      //  if(nNeutral==0) decaymode[pdgId] = 0;
      //  if(nNeutral==1) decaymode[pdgId] = 1;
      //  if(nNeutral==2) decaymode[pdgId] = 2;
      //  if(nNeutral==3) decaymode[pdgId] = 3;
      //  if(nNeutral==4) decaymode[pdgId] = 4;
      //  if(nNeutral>=5) decaymode[pdgId] = 5;
      //}else if(nCharged==3){
      //  decaymode[pdgId] = std::max(nNeutral,9);
      //  if(nNeutral==0) decaymode[pdgId] = 10;
      //  if(nNeutral==1) decaymode[pdgId] = 11;
      //  if(nNeutral==2) decaymode[pdgId] = 12;
      //  if(nNeutral==3) decaymode[pdgId] = 13;
      //  if(nNeutral>=4) decaymode[pdgId] = 14;
      //}else if(nCharged==5){
      //  decaymode[pdgId] = 20;
      }else{
        std::cout << "genDecayMode: did not recognize tau decay: "<<std::endl;
        for(int idau=0; idau < (int)particle.nDau(); idau++)
          std::cout << abs(particle.dau()[idau]) << ", ";
        std::cout << std::endl;
        decaymode[pdgId] = -1;
      }
    }
...
//     std::cout << "Ntuplizer: ";
//     for( int itau = 0; itau < (int)m_genParticle.tauvispt->size(); ++itau ){
//       std::cout << " " << m_genParticle.tauvispt->at(itau);
//     }
//     std::cout << std::endl;
//     std::cout << "genDM:     ";
//   }
  ...
  for(std::map<int, TLorentzVector>::iterator it = gentaus.begin(); it!=gentaus.end(); ++it){
    //std::cout << " " << (*it).second.Pt();
    Float_t dr = tau.DeltaR((*it).second.Eta(),(*it).second.Phi());
    if(dr < min_dR){
      min_dR   = dr;
      dm       = decaymode[(*it).first];
      genvispt = (*it).second.Pt();
      genvism  = (*it).second.M();
    }
  }
  //if(gentaus.size()>0) std::cout << std::endl;
...
//   TLorentzVector met_tlv_shifted; //= met.tlv();
